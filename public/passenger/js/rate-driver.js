(function(){
  ...
  async function fetchRideDetails(){
    try {
      ...
      console.log('[Rate Driver] Fetching ride details for booking:', bookingId);

      const response = await fetch(`http://localhost:3000/api/reviews/ride-details/${bookingId}`);
      
      if(!response.ok){
        console.error('[Rate Driver] Failed to fetch ride details:', response.status);
  ...
  function renderRideDetails(rideData){
    ...
    el('driver-name').textContent = (d.firstName || 'Unknown') + ' ' + (d.lastName || 'Driver');
    el('driver-vehicle').textContent = rideData.vehicle ? (rideData.vehicle.model || rideData.vehicle.licensePlate || 'Vehicle') : 'Vehicle';
    ...
    if(reviewAlreadySubmitted){
      ...
      if(existingReview){
        ...
        infoDiv.style.cssText = 'background:#e8f5e9;color:#2e7d32;padding:12px;border-radius:6px;margin:12px 0;border-left:4px solid #2e7d32';
        infoDiv.textContent = '✓ You have already submitted a review for this ride on ' + new Date(existingReview.created_at).toLocaleDateString();
